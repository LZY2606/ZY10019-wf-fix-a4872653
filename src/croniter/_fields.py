...
Turns a cron expression string into the normalized ``expanded`` structure
(list of allowed values per field) plus the side tables
(``nth_weekday_of_month``, ``nearest_weekday``) consumed by the date
matcher. Also owns the ``H``/``R`` hash/random expansion and a small
expansion cache: results for the same expression and seed (``hash_id``)
are safe to reuse, while ``R`` (random) expressions always bypass the
cache so each instance re-rolls its schedule.
"""
...
from ._errors import CroniterBadCronError, CroniterNotAlphaError, CroniterUnsupportedSyntaxError
...
# Aliases for well-known schedules. The second form of each pair is used
# when a hash_id is given, spreading the schedule with hashed fields.
EXPR_ALIASES = {
...
def alphaconv(cls, index, key, expressions):
    try:
...
def get_low_from_current_date_number(cls, field_index, step, from_timestamp, tzinfo=None):
...
def _has_random_field(expr_format, hash_id):
    """True when the expression contains an ``R`` (random) hash field."""
    efl = expr_format.lower()
    alias = EXPR_ALIASES.get(efl)
    if alias is not None:
        efl = alias[1 if hash_id is not None else 0]
    for field in efl.split():
        m = hash_expression_re.match(field)
        if m and m.group("hash_type") == "r":
            return True
    return False


def _copy_expansion(result):
    """Return a defensive copy of a cached expansion result.

    Callers (including ``croniter`` instances) may mutate the returned
    structures, so cached results must never be handed out directly.
    """
    expanded, nth_weekday_of_month, expressions, nearest_weekday = result
    return (
        [list(field) for field in expanded],
        {key: set(values) for key, values in nth_weekday_of_month.items()},
        list(expressions),
        set(nearest_weekday),
    )


MARKER_CACHE_MISS = object()
...
def expand_expression_cached(
    ...
    Expansion is deterministic for a given (class, expression, hash_id,
    timestamps, strict flags) combination, so identical expressions with the
    same seed can share one cached result (handed out as defensive copies).
    ``R`` (random) fields are excluded from caching: their schedule must be
    re-rolled on every expansion and must never be frozen by the cache.
    """
    kwargs = dict(
        hash_id=hash_id,
    ...
    if _has_random_field(expr_format, hash_id):
        return expand_expression(cls, expr_format, **kwargs)
    try:
        ...
        cached = _expansion_cache.get(key, MARKER_CACHE_MISS)
    except TypeError:
        # Unhashable key component (e.g. an exotic tzinfo); skip the cache.
        return expand_expression(cls, expr_format, **kwargs)
    if cached is MARKER_CACHE_MISS:
        cached = expand_expression(cls, expr_format, **kwargs)
        if len(_expansion_cache) >= _EXPANSION_CACHE_MAXSIZE:
            _expansion_cache.pop(next(iter(_expansion_cache)))
        _expansion_cache[key] = cached
